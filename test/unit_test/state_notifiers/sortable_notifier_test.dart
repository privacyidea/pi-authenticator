...
void _testSortableNotifier() {
  group('SortableNotifier test', () {
    test('handleNewList', () async {
      ...
      final MockTokenFolderRepository mockTokenFolderRepository =
          MockTokenFolderRepository();
      final MockTokenRepository mockTokenRepository = MockTokenRepository();
      TokenFolderState tokenFolderState = const TokenFolderState(
        folders: [
          TokenFolder(label: 'Folder 1', folderId: 1),
      ...
      when(mockTokenFolderRepository.saveState(any)).thenAnswer((
        newState,
      ) async {
        tokenFolderState =
            newState.positionalArguments.first as TokenFolderState;
        return true;
      });
