...
void main() {
  ...
  group('FirebaseUtils SecureStorage', () {
    group('getCurrentFirebaseToken', () {
      ...
      test(
        'migrates token from legacy storage if new one is not available',
        () async {
          when(
            mockStorage.read(key: fullCurrentNewKey),
          ).thenAnswer((_) async => null);
          when(
            mockLegacyStorage.read(key: fullCurrentLegacyKey),
          ).thenAnswer((_) async => 'legacy_token');
          when(
            mockStorage.write(key: fullCurrentNewKey, value: 'legacy_token'),
          ).thenAnswer((_) async {});
          when(
            mockLegacyStorage.delete(key: fullCurrentLegacyKey),
          ).thenAnswer((_) async {});
    ...
    group('getNewFirebaseToken', () {
      test('migrates token from legacy storage', () async {
        when(
          mockStorage.read(key: fullNewNewKey),
        ).thenAnswer((_) async => null);
        when(
          mockLegacyStorage.read(key: fullNewLegacyKey),
        ).thenAnswer((_) async => 'legacy_new_token');
        when(
          mockStorage.write(key: fullNewNewKey, value: 'legacy_new_token'),
        ).thenAnswer((_) async {});
        when(
          mockLegacyStorage.delete(key: fullNewLegacyKey),
        ).thenAnswer((_) async {});
    ...
    test('setCurrentFirebaseToken writes to new storage', () async {
      when(
        mockStorage.write(key: fullCurrentNewKey, value: 'test_token'),
      ).thenAnswer((_) async {});
    ...
    test('setNewFirebaseToken writes to new storage', () async {
      when(
        mockStorage.write(key: fullNewNewKey, value: 'test_token'),
      ).thenAnswer((_) async {});
