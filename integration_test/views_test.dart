...
void main() {
  ...
  setUp(() {
    ...
    var state = PushRequestState(
      pushRequests: [],
      knownPushRequests: CustomIntBuffer(list: []),
    );
    when(mockPushRequestRepository.saveState(any)).thenAnswer((
      invocation,
    ) async {
      final arguments = invocation.positionalArguments;
    ...
    when(mockPushRequestRepository.addRequest(any)).thenAnswer((
      invocation,
    ) async {
      final arguments = invocation.positionalArguments;
...
Future<void> _licensesViewTest(WidgetTester tester) async {
  ...
  expect(
    find.text(ApplicationCustomization.defaultCustomization.websiteLink),
    findsOneWidget,
  );
