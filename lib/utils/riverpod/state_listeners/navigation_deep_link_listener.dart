...
class NavigationDeepLinkListener extends DeepLinkListener {
  ...
  static void _onNewState(AsyncValue<DeepLink>? previous, AsyncValue<DeepLink> next) {
    next.whenData((next) async {
      try {
        await NavigationSchemeProcessor.processUriByAny(next.uri, context: _context, fromInit: next.fromInit);
      } catch (e, s) {
        Logger.error(
          '[NavigationDeepLinkListener] Failed to process uri: ${next.uri} (fromInit: ${next.fromInit})',
          error: e,
          stackTrace: s,
        );
      }
