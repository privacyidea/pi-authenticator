import 'package:intl/intl.dart' as intl;

import 'app_localizations.dart';

// ignore_for_file: type=lint

/// The translations for German (`de`).
class AppLocalizationsDe extends AppLocalizations {
  AppLocalizationsDe([String locale = 'de']) : super(locale);

  @override
  String get a11yAddFolderButton => 'Ordner hinzufügen';

  @override
  String get a11yAddTokenManuallyButton => 'Token manuell hinzufügen';

  @override
  String get a11yCloseSearchTokensButton => 'Suche schließen';

  @override
  String get a11yLicensesButton => 'Lizenzen';

  @override
  String get a11yPushTokensButton => 'Push-Token';

  @override
  String get a11yScanQrCodeButton => 'QR-Code scannen';

  @override
  String get a11yScanQrCodeViewActive => 'Scan-QR-Code-Ansicht. Cammera ist aktiv';

  @override
  String get a11yScanQrCodeViewFlashlightOff => 'Tippen Sie zum Einschalten der Taschenlampe.';

  @override
  String get a11yScanQrCodeViewFlashlightOn => 'Tippen Sie zum Ausschalten der Taschenlampe.';

  @override
  String get a11yScanQrCodeViewGallery => 'Galerie öffnen';

  @override
  String get a11yScanQrCodeViewInactive => 'Scan-QR-Code-Ansicht. Cammera ist nicht aktiv';

  @override
  String get a11ySearchTokensButton => 'Token suchen';

  @override
  String get a11ySettingsButton => 'Einstellungen';

  @override
  String get accept => 'Akzeptieren';

  @override
  String get addANewFolder => 'Neuen Ordner anlegen';

  @override
  String get addSystemInfo => 'Systeminfos hinzufügen';

  @override
  String get addToken => 'Token hinzufügen';

  @override
  String get additionalErrorMessage => 'Optionale Nachricht';

  @override
  String get algorithm => 'Algorithmus';

  @override
  String algorithmUnsupported(String algorithm) {
    return 'Der Algorithmus $algorithm wird nicht unterstützt';
  }

  @override
  String get allTokensSynchronized => 'Alle Token wurden synchronisiert.';

  @override
  String get asFile => 'Als Datei';

  @override
  String get asQrCode => 'Als QR-Code';

  @override
  String get askLogSendedDescription => 'Haben Sie das Protokoll gesendet, und möchten Sie es jetzt löschen?';

  @override
  String get authNotSupportedBody => 'Diese Aktion erfordert, dass auf dem Gerät ein Passwort oder Biometrie eingerichtet ist.';

  @override
  String get authNotSupportedTitle => 'Gerätepasswort oder Biometrie wird benötigt';

  @override
  String get authToAcceptPushRequest => 'Bitte authentifizieren Sie sich, um die Anfrage anzunehmen.';

  @override
  String get authToDeclinePushRequest => 'Bitte authentifizieren Sie sich, um die Anfrage abzulehnen.';

  @override
  String get authenticateToShowOtp => 'Bitte authentifizieren Sie sich, um das Einmalpasswort anzuzeigen.';

  @override
  String get authenticateToUnLockToken => 'Bitte authentifizieren Sie sich, um den Sperrstatus des Tokens zu ändern.';

  @override
  String get authenticationRequest => 'Authentifizierung';

  @override
  String get biometricHint => 'Authentifizierung wird benötigt';

  @override
  String get biometricNotRecognized => 'Biometrie wurde nicht erkannt, bitte versuchen Sie es erneut';

  @override
  String get biometricRequiredTitle => 'Biometrie ist nicht eingerichtet';

  @override
  String get biometricSuccess => 'Authentifizierung erfolgreich';

  @override
  String get butDiscardIt => 'aber verwerfen';

  @override
  String get cancel => 'Abbrechen';

  @override
  String get checkServerCertificate => 'Bitte überprüfen Sie das Serverzertifikat';

  @override
  String get checkYourNetwork => 'Bitte überprüfen Sie Ihre Netzwerkverbindung und versuchen Sie es erneut.';

  @override
  String get clearErrorLog => 'Löschen';

  @override
  String get clipboardEmpty => 'Zwischenablage ist leer';

  @override
  String get confirmDeletion => 'Löschen bestätigen';

  @override
  String confirmDeletionOf(String name) {
    return 'Sind Sie sicher dass Sie $name löschen möchten?';
  }

  @override
  String get confirmFolderDeletionHint => 'Das Löschen eines Ordners hat keine Auswirkungen auf die Token, die sich darin befinden.\nDie Token werden in die Hauptliste verschoben.';

  @override
  String get confirmPassword => 'Passwort bestätigen';

  @override
  String get confirmTokenDeletionHint => 'Unter Umständen können Sie sich nicht mehr einloggen, wenn Sie diesen Token löschen.\nBitte stellen Sie sicher, dass Sie sich ohne diesen Token in den dazugehörigen Account einloggen können.';

  @override
  String get confirmation => 'Confirmation';

  @override
  String get connectionFailed => 'Verbindung fehlgeschlagen.';

  @override
  String get container => 'Container';

  @override
  String get containerAlreadyExists => 'Container existiert bereits';

  @override
  String get containerDetails => 'Containerdetails';

  @override
  String get containerSerial => 'Containerserial';

  @override
  String get containerSyncUrl => 'Container Sync Url';

  @override
  String get continueButton => 'Weiter';

  @override
  String get copyOTPToClipboard => 'OTP in die Zwischenablage kopieren';

  @override
  String get couldNotConnectToServer => 'Konnte keine Verbindung zum Server herstellen.';

  @override
  String get couldNotSignMessage => 'Nachricht konnte nicht signiert werden.';

  @override
  String get counter => 'Zähler';

  @override
  String get create => 'Erstellen';

  @override
  String get createdAt => 'Erstellt am';

  @override
  String get creator => 'Ersteller';

  @override
  String get dayPasswordValidFor => 'Gültig für';

  @override
  String get dayPasswordValidUntil => 'Gültig bis';

  @override
  String get decline => 'Ablehnen';

  @override
  String get declineIt => 'ablehnen';

  @override
  String get decrypt => 'Entschlüsseln';

  @override
  String get decryptErrorButtonDelete => 'Löschen.';

  @override
  String get decryptErrorButtonRetry => 'Wiederholen';

  @override
  String get decryptErrorButtonSendError => 'Fehler senden';

  @override
  String get decryptErrorContent => 'Leider konnten Ihre Token nicht entschlüsselt werden. Das deutet darauf hin, dass der Verschlüsselungsschlüssel nicht mehr verfügbar ist. Sie können es erneut versuchen oder die App Daten löschen. Dabei werden alle Token aus der App geschlöscht.';

  @override
  String get decryptErrorDeleteConfirmationContent => 'Sind Sie sicher, dass Sie die App Daten löschen möchten?';

  @override
  String get decryptErrorTitle => 'Entschlüsselung fehlgeschlagen';

  @override
  String get delete => 'Löschen';

  @override
  String get deleteContainerDialogContent => 'Wenn Sie diesen Container löschen, wird das Smartphone vom privacyIDEA Server getrennt und die Token dieses Containers werden unbrauchbar. Stellen Sie vor dem Löschen sicher, dass die entsprechenden Token nicht mehr benötigt werden!';

  @override
  String deleteContainerDialogTitle(String serial) {
    return 'Container $serial löschen';
  }

  @override
  String get deleteLockedToken => 'Bitte authentifizieren Sie sich, um den gesperrten Token zu löschen.';

  @override
  String get details => 'Details';

  @override
  String get deviceCredentialsRequiredTitle => 'Gerätepasswort ist nicht eingerichtet';

  @override
  String get deviceCredentialsSetupDescription => 'Setzen Sie bitte ein Gerätepasswort in den Einstellungen';

  @override
  String get digits => 'Ziffern';

  @override
  String get dismiss => 'Schließen';

  @override
  String get done => 'Fertig';

  @override
  String get edit => 'Bearbeiten';

  @override
  String get editLockedToken => 'Bitte authentifizieren Sie sich, um den gesperrten Token zu bearbeiten.';

  @override
  String get editToken => 'Token bearbeiten';

  @override
  String get enablePolling => 'Aktives Stellen von Push-Anfragen';

  @override
  String get encoding => 'Kodierung';

  @override
  String get enterDetailsForToken => 'Neuen Token konfigurieren';

  @override
  String get enterLink => 'Link eingeben';

  @override
  String get enterPasswordToEncrypt => 'Geben Sie ein Passwort ein, um die Tokens zu verschlüsseln. Dieses Passwort wird benötigt, um die Tokens zu importieren.';

  @override
  String get errorLogCleared => 'Fehlerprotokoll gelöscht.';

  @override
  String get errorLogEmpty => 'Das Fehlerprotokoll ist leer.';

  @override
  String get errorLogTitle => 'Fehlerprotokoll';

  @override
  String get errorMailBody => 'Die Fehlerprotokolldatei ist angehängt.\nSie können diesen Text durch zusätzliche Informationen über den Fehler ersetzen.';

  @override
  String get errorMissingPrivateKey => 'Privater Schlüssel fehlt';

  @override
  String errorRollOutFailed(String name) {
    return 'Ausrollen von $name ist fehlgeschlagen.';
  }

  @override
  String errorRollOutNoConnectionToServer(String name) {
    return 'Der Rollout von Token $name ist fehlgeschlagen, der Server konnte nicht erreicht werden.';
  }

  @override
  String get errorRollOutNotPossibleAnymore => 'Das Ausrollen dieses Tokens ist nicht mehr möglich.';

  @override
  String get errorRollOutSSLHandshakeFailed => 'SSL-Handshake fehlgeschlagen. Roll-out nicht möglich.';

  @override
  String errorRollOutUnknownError(String e) {
    return 'Ein unbekannter Fehler ist aufgetreten. Aurollen nicht möglich: $e';
  }

  @override
  String get errorSavingFile => 'Fehler beim Speichern der Datei';

  @override
  String get errorSynchronizationNoNetworkConnection => 'Die Synchronisation ist fehlgeschlagen, da der privacyIDEA Server nicht erreicht werden konnte.';

  @override
  String errorTokenExpired(String name) {
    return 'Der Token $name ist abgelaufen.';
  }

  @override
  String errorUnlinkingPushToken(String label) {
    return 'Entkoppeln des Push Tokens $label fehlgeschlagen.';
  }

  @override
  String errorWhenPullingChallenges(String name) {
    return 'Fehler beim Abrufen der Authentifizierungsanfragen von $name';
  }

  @override
  String get exampleUrl => 'Bitte geben Sie eine gültige URL ein wie: \"https://example.com\"';

  @override
  String get expandLockedFolder => 'Bitte authentifizieren Sie sich, um den gesperrten Ordner zu öffnen.';

  @override
  String get export => 'Exportieren';

  @override
  String get exportAllTokens => 'Alle Tokens exportieren';

  @override
  String get exportLockedTokenReason => 'Bitte authentifizieren Sie sich, um gesperrte Tokens zu exportieren.';

  @override
  String get exportNonPrivacyIDEATokens => 'Nicht-privacyIDEA-Token exportieren';

  @override
  String get exportOneMore => 'Noch eins';

  @override
  String get exportTokens => 'Tokens exportieren';

  @override
  String get exportingTokens => 'Tokens werden exportiert...';

  @override
  String failedToFinalizeContainer(String serial) {
    return 'Der Container $serial konnte nicht abgeschlossen werden';
  }

  @override
  String failedToLoad(String name) {
    return 'Fehlgeschlagen zu laden: $name';
  }

  @override
  String failedToSyncContainer(String serial) {
    return 'Container $serial konnte nicht synchronisiert werden';
  }

  @override
  String get feedback => 'Feedback';

  @override
  String get feedbackDescription => 'Wenn Sie Fragen, Anregungen oder Probleme haben, lassen Sie es uns wissen.';

  @override
  String get feedbackHint => 'Es öffnet sich eine vorgefertigte E-Mail, die Sie an uns senden können. Falls gewünscht, werden Informationen über Ihr Gerät und die Version der Anwendung hinzugefügt. Vor dem Versenden können Sie die E-Mail überprüfen und bearbeiten.';

  @override
  String get feedbackPrivacyPolicy1 => 'Mit dem Senden des Feedbacks stimmen Sie unserer ';

  @override
  String get feedbackPrivacyPolicy2 => 'Datenschutzerklärung';

  @override
  String get feedbackPrivacyPolicy3 => ' zu.';

  @override
  String get feedbackSentDescription => 'Vielen Dank für Ihre Hilfe bei der Verbesserung dieser App!';

  @override
  String get feedbackSentTitle => 'Feedback gesendet';

  @override
  String get feedbackTitle => 'Ihr Feedback ist immer willkommen!';

  @override
  String get fileSavedToDownloadsFolder => 'Datei wurde im Download-Ordner gespeichert';

  @override
  String get finalizationState => 'Status der Finalisierung';

  @override
  String get finalizeContainerFailed => 'Container finalisieren fehlgeschlagen';

  @override
  String get firebaseToken => 'Firebase Token';

  @override
  String get folderName => 'Ordnername';

  @override
  String get generatingPhonePart => 'Generiere Telefonanteil';

  @override
  String get gitHubButton => 'Diese App ist Open Source\nBesuchen Sie uns auf GitHub';

  @override
  String get goToSettingsButton => 'Gehe zu Einstellungen';

  @override
  String get goToSettingsDescription => 'Authentifizierung durch Gerätepasswort oder Biometrie ist nicht eingerichtet. Bitte aktivieren Sie dies in den Geräteeinstellungen.';

  @override
  String get grantCameraPermissionDialogButton => 'Berechtigung erteilen';

  @override
  String get grantCameraPermissionDialogContent => 'Um QR-Codes zu scannen, benötigt die App Zugriff auf die Kamera.';

  @override
  String get grantCameraPermissionDialogPermanentlyDenied => 'Sie haben die Berechtigung für den Kamerazugriff permanent verweigert. Bitte aktivieren Sie die Berechtigung in den Einstellungen ihres Smartphones.';

  @override
  String get grantCameraPermissionDialogTitle => 'Kamera-Berechtigung erforderlich';

  @override
  String get handshakeFailed => 'Handshake fehlgeschlagen';

  @override
  String get hidePushTokens => 'Push-Token ausblenden';

  @override
  String get hidePushTokensDescription => 'Push-Token aus der Token-Liste ausblenden. Dadurch werden die Token nicht gelöscht und sind weiterhin auf einem separaten Bildschirm sichtbar.';

  @override
  String get imageUrl => 'Bild URL';

  @override
  String importConflictToken(int count) {
    String _temp0 = intl.Intl.pluralLogic(
      count,
      locale: localeName,
      other: 'Es bestehen Konflikte mit bereits vorhandenen Token.\nBitte wählen Sie die Token aus, die Sie behalten möchten.',
      one: 'Es besteht ein Konflikt mit bereits vorhandenen Token.\nBitte wählen Sie aus, welches Sie behalten möchten.',
      zero: 'Es besteht kein Konflikt mit bereits existierenden Token.',
    );
    return '$_temp0';
  }

  @override
  String importExistingToken(int count) {
    String _temp0 = intl.Intl.pluralLogic(
      count,
      locale: localeName,
      other: 'Es wurden $count Token gefunden, die sich bereits in der Anwendung befinden.',
      one: 'Es wurde ein Token gefunden, das sich bereits in der Anwendung befindet.',
      zero: 'Es wurde kein Token gefunden, das sich bereits in der App befindet.',
    );
    return '$_temp0';
  }

  @override
  String get importExportTokens => 'Token importieren/exportieren';

  @override
  String importFailedToken(int count) {
    String _temp0 = intl.Intl.pluralLogic(
      count,
      locale: localeName,
      other: 'Der Import von $count Token ist fehlgeschlagen.',
      one: 'Importieren eines Tokens fehlgeschlagen.',
      zero: 'Kein Token konnte nicht importiert werden.',
    );
    return '$_temp0';
  }

  @override
  String get importHint2FAS => 'Wählen Sie das 2FAS-Backup aus.\nFalls Sie kein Backup haben, erstellen Sie eins in der 2FAS-App. Wir empfehlen die Verwendung eines Passworts.';

  @override
  String get importHintAegisBackupFile => 'Wähle dein Aegis-Export (.json) aus.\nWenn Sie keinen Export haben, erstellen Sie bitte eins über das Einstellungen Menu in der Aegis-App. Wir empfehlen die Verwendung eines Passworts.';

  @override
  String get importHintAegisLink => 'Geben Sie den Link ein, den Sie erhalten, wenn Sie Einträge aus Aegis übertragen.';

  @override
  String get importHintAegisQrScan => 'Scannen Sie den QR-Code, den Sie erhalten, wenn Sie Einträge aus Aegis übertragen.';

  @override
  String get importHintAuthenticatorProFile => 'Um ein Backup der Authenticator Pro-App zu erstellen navigieren Sie zu den Einstellungen und tippen Sie auf \"Automatische Sicherung\". Wählen Sie einen Speicherort und setzen Sie ein Passwort. Anschließend drücken Sie auf \"Jetzt sichern\" um die Token zu exportieren.';

  @override
  String get importHintFreeOtpPlusFile => 'Um ein Backup der FreeOTP+ App zu erstellen, tippen Sie auf die drei Punkte in der oberen rechten Ecke und wählen Sie \"Exportieren\". Sie können zwischen dem JSON- und dem URI-Format wählen. Wir empfehlen, das Backup nach dem Importieren zu löschen, da es nicht verschlüsselt ist.';

  @override
  String get importHintFreeOtpPlusQrScan => 'Scannen Sie den QR-Code, den Sie erhalten, wenn Sie auf die drei Punkte in der Kachel des Tokens drücken, und wählen Sie \"QR-Code teilen\".';

  @override
  String get importHintGoogleQrFile => 'Wählen Sie eine Bilddatei mit dem QR-Code, den Sie erhalten, wenn Sie Ihre Konten aus dem Google Authenticator exportieren.\n!! Der QR-Code enthält die Token in unverschlüsselter Form. Es ist deshalb nicht sicher, diesen länger als nötig aufzubewahren !!';

  @override
  String get importHintGoogleQrScan => 'Scannen Sie den QR-Code, den Sie erhalten, wenn Sie Ihre Konten aus Google Authenticator exportieren.';

  @override
  String get importHintPrivacyIdeaFile => 'Um ein Backup zu erstellen, gehen Sie zu den Einstellungen und tippen auf \"Exportieren\". Wählen Sie \"Als Datei\" aus, wählen Sie die Token aus, die Sie exportieren möchten. Anschließend tippen Sie auf \"Exportieren\" und setzen Sie ein Passwort. Der Speicherort ist der Download-Ordner auf Ihrem Gerät.';

  @override
  String get importHintPrivacyIdeaQrScan => 'Um QR-Codes der Token zu erstellen, navigieren Sie zu den Einstellungen und tippen auf \"Exportieren\". Wählen Sie dann \"Als QR-Code\" und tippen Sie auf den zu exportierenden Token. Diese Variante ist nur für die direkte Übertragung auf ein anderes Gerät geeignet, da der QR-Code nicht verschlüsselt ist.';

  @override
  String importNTokens(int count) {
    String _temp0 = intl.Intl.pluralLogic(
      count,
      locale: localeName,
      other: '$count Token importieren',
      one: 'Ein Token importieren',
      zero: 'Keine Token importieren',
    );
    return '$_temp0';
  }

  @override
  String importNewToken(int count) {
    String _temp0 = intl.Intl.pluralLogic(
      count,
      locale: localeName,
      other: 'Es wurden $count neue Token gefunden, die importiert werden können.',
      one: 'Es wurde ein neues Token gefunden, das importiert werden kann',
      zero: 'Es wurde kein neues Token gefunden.',
    );
    return '$_temp0';
  }

  @override
  String get importTokens => 'Token importieren';

  @override
  String get importantInformationTitle => 'Wichtige Informationen';

  @override
  String get importedVia => 'Importiert über';

  @override
  String get increaseCounter => 'Zähler erhöhen';

  @override
  String internalServerError(String code) {
    return 'Interner Serverfehler ($code)';
  }

  @override
  String get introAddFolder => 'Sie können Ordner erstellen, um Ihre Token zu organisieren.';

  @override
  String get introAddTokenManually => 'Wenn Sie keinen QR-Code scannen möchten, können Sie Token auch manuell hinzufügen.';

  @override
  String get introDragToken => 'Reorganisieren Sie Ihre Token, indem Sie sie einige Sekunden lang drücken und dann an die gewünschte Position ziehen.';

  @override
  String get introEditToken => 'Hier können Sie den Namen des Tokens bearbeiten und einige Details einsehen.';

  @override
  String get introHidePushTokens => 'Deine Push-Token sind jetzt versteckt.\nAber du kannst sie immer noch auf dem Bildschirm mit den Push-Token sehen.';

  @override
  String get introLockToken => 'Um die Sicherheit noch weiter zu erhöhen, können Sie Token sperren.\nDer Token kann dann erst nach der Authentifizierung verwendet werden.';

  @override
  String get introPollForChallenges => 'Sie können neue Push-Anmeldungen abfragen, indem Sie die Liste der Token nach unten ziehen.';

  @override
  String get introScanQrCode => 'Sie können QR-Codes scannen, um Token hinzuzufügen.\nWir unterstützen alle gängigen Two-Factor-Authentication Token und auch die privacyIDEA Token.';

  @override
  String get introTokenSwipe => 'Wischen Sie Token nach links, um die verfügbaren Aktionen zu sehen.';

  @override
  String invalidBackupFile(String appName) {
    return 'Die ausgewählte Datei ist kein gültiges Backup von $appName.';
  }

  @override
  String invalidLink(String appName) {
    return 'Der eingegebene Link ist kein gültiger Token von $appName, oder er wird nicht unterstützt.';
  }

  @override
  String invalidQrFile(String appName) {
    return 'Die ausgewählte Datei enthällt kein gültigen QR-Code von $appName.';
  }

  @override
  String invalidQrScan(String appName) {
    return 'Der gescannte QR-Code ist kein gültiges Backup von $appName.';
  }

  @override
  String get invalidUrl => 'Ungültige URL';

  @override
  String invalidValue(String parameter, String type, String value) {
    return 'Der $type \'$value\' ist für \'$parameter\' nicht gültig.';
  }

  @override
  String invalidValueIn(String map, String parameter, String type, String value) {
    return 'Der $type \'$value\' ist für \'$parameter\' in \'$map\' ungültig';
  }

  @override
  String get isExpotableQuestion => 'Ist exportierbar?';

  @override
  String get isPiTokenQuestion => 'Ist ein privacyIDEA-Token?';

  @override
  String get issuer => 'Aussteller';

  @override
  String issuerLabel(String name) {
    return 'Aussteller: $name';
  }

  @override
  String get language => 'Sprache';

  @override
  String get legacySigningErrorMessage => 'Der Token wurde in einer veralteten Version der App erstellt, was zu Problemen bei der Verwendung führen kann. Es wird empfohlen, einen neuen Push-Token zu erstellen, wenn das Problem weiterhin besteht!';

  @override
  String legacySigningErrorTitle(String tokenLabel) {
    return 'Bei der Verwendung des veralteten Tokens ist ein Fehler aufgetreten: $tokenLabel';
  }

  @override
  String get licensesAndVersion => 'Lizenzen und Version';

  @override
  String get linkMustOtpAuth => 'Der Link muss mit otpauth:// beginnen';

  @override
  String get linkedContainer => 'Verknüpfter Container';

  @override
  String get lock => 'Sperren';

  @override
  String get lockOut => 'Biometrie ist deaktiviert. Bitte sperren und entsperren Sie Ihren Bildschirm um diese zu aktivieren.';

  @override
  String get logMenu => 'Log-Menü';

  @override
  String get malformedData => 'Fehlerhafte Daten';

  @override
  String get markQrCode => 'QR-Code markieren';

  @override
  String missingRequiredParameter(String parameter) {
    return 'Der Wert für den Parameter [$parameter] ist erforderlich, fehlt aber.';
  }

  @override
  String missingRequiredParameterIn(String map, String parameter) {
    return 'Der Wert für den Parameter [$parameter] ist erforderlich, fehlt aber in \"$map\".';
  }

  @override
  String mustNotBeEmpty(String field) {
    return '$field darf nicht leer sein';
  }

  @override
  String get name => 'Name';

  @override
  String get no => 'Nein';

  @override
  String get noFbToken => 'Kein Firebase Token vorhanden';

  @override
  String get noMailAppDescription => 'Auf diesem Gerät ist keine E-Mail-App installiert oder initialisiert, bitte versuchen Sie es erneut, wenn Sie eine E-Mail-Nachricht senden können.';

  @override
  String get noMailAppTitle => 'Keine Mail-App gefunden';

  @override
  String get noNetworkConnection => 'Keine Netzwerkverbindung.';

  @override
  String get noPublicKey => 'Kein öffentlicher Schlüssel vorhanden';

  @override
  String get noResultText1 => 'Tippe auf das ';

  @override
  String get noResultText2 => ' Icon um loszulegen!';

  @override
  String get noResultTitle => 'Keine Token vorhanden.';

  @override
  String get noTokenToExport => 'Kein Token zum Exportieren verfügbar';

  @override
  String get noTokenToImport => 'Kein Token zum Importieren gefunden';

  @override
  String get notAnInteger => 'Der Wert ist keine Ganzzahl.';

  @override
  String get notAnNumber => 'Der Wert ist keine Zahl.';

  @override
  String get ok => 'Ok';

  @override
  String get open => 'Öffnen';

  @override
  String get originApp => 'Ursprungs-App';

  @override
  String get originDetails => 'Angaben zur Herkunft';

  @override
  String otpValueCopiedMessage(String otpValue) {
    return 'Passwort \"$otpValue\" wurde in Zwischenablage kopiert.';
  }

  @override
  String get password => 'Passwort';

  @override
  String get passwordCannotBeEmpty => 'Das Passwort darf nicht leer sein';

  @override
  String get passwordCannotContainWhitespace => 'Das Passwort darf keine Leerzeichen enthalten';

  @override
  String get passwordMustBeAtLeast8Characters => 'Das Passwort muss mindestens 8 Zeichen lang sein';

  @override
  String get passwordMustContainLowercaseLetter => 'Das Passwort muss einen Kleinbuchstaben enthalten';

  @override
  String get passwordMustContainNumber => 'Das Passwort muss eine Zahl enthalten';

  @override
  String get passwordMustContainSpecialCharacter => 'Das Passwort muss ein Sonderzeichen enthalten';

  @override
  String get passwordMustContainUppercaseLetter => 'Das Passwort muss einen Großbuchstaben enthalten';

  @override
  String get passwordsDoNotMatch => 'Die Passwörter stimmen nicht überein';

  @override
  String get patchNotesBugFixes => 'Fehlerbehebungen';

  @override
  String get patchNotesDialogTitle => 'Was ist neu?';

  @override
  String get patchNotesImprovements => 'Verbesserungen';

  @override
  String get patchNotesNewFeatures => 'Neue Funktionen';

  @override
  String get patchNotesV4_3_0NewFeatures1 => 'Unterstützung für den Import von Token von Google, Aegis und 2FAS Authenticator hinzugefügt. Weitere Importquellen werden in Zukunft hinzugefügt.';

  @override
  String get patchNotesV4_3_0NewFeatures2 => 'Feedback-Option zu den Einstellungen hinzugefügt.';

  @override
  String get patchNotesV4_3_0NewFeatures3 => 'Push-Token können jetzt aus der Token-Liste ausgeblendet werden.';

  @override
  String get patchNotesV4_3_0NewFeatures4 => 'Es wurden Einführungen hinzugefügt, um neuen Benutzern den Einstieg zu erleichtern.';

  @override
  String get patchNotesV4_3_0NewFeatures5 => 'Sie können jetzt nach Token suchen, indem Sie auf die Lupe in der oberen rechten Ecke tippen.';

  @override
  String get patchNotesV4_3_0NewFeatures6 => 'Ab Android 12 kann für einen Token ein Widget auf dem Homescreen erstellt werden.';

  @override
  String get patchNotesV4_3_1BugFix1 => 'Ein Problem wurde behoben, bei dem der otp-Wert nach der Authentifizierung auf einigen Geräten nicht angezeigt wurde.';

  @override
  String get patchNotesV4_3_1Improvement1 => 'Der QR-Code-Scanner wurde verbessert.';

  @override
  String get patchNotesV4_4_0Improvement1 => 'Es wurden weitere Importquellen hinzugefügt.';

  @override
  String get patchNotesV4_4_0Improvement2 => 'Die erkennung von QR-Codes aus Bilddateien wurde verbessert.';

  @override
  String get patchNotesV4_4_0NewFeatures1 => 'Es ist nun möglich, Token zu exportieren, bei denen sichergestellt werden kann, dass es sich nicht um privacyIDEA Token handelt. Derzeit kann nicht ausgeschlossen werden, dass über den QR-Code-Scanner hinzugefügte Token von privacyIDEA stammen. Die Differenzierung wird in zukünftigen Versionen verbessert.';

  @override
  String get patchNotesV4_4_0NewFeatures2 => 'Unterstützung für privacyIDEA\'s \"require presence\" hinzugefügt.';

  @override
  String get patchNotesV4_4_2Improvement1 => 'Taschenlampenunterstützung für das Scannen von QR-Codes wurde zugefügt.';

  @override
  String get patchNotesV4_4_2NewFeatures1 => 'Token können jetzt per Copy & Paste eingefügt werden.';

  @override
  String get patchNotesV4_4_2NewFeatures2 => 'Galerie-Unterstützung für das Scannen von QR-Codes hinzugefügt.';

  @override
  String get period => 'Periode';

  @override
  String get phonePart => 'Telefonanteil:';

  @override
  String piServerCode(String code) {
    return 'PI-Server-Code: $code';
  }

  @override
  String get pleaseEnterANameForThisToken => 'Bitte geben Sie einen Namen ein.';

  @override
  String get pleaseEnterASecretForThisToken => 'Bitte geben Sie ein Geheimnis ein.';

  @override
  String get pollingFailed => 'Abfrage fehlgeschlagen.';

  @override
  String pollingFailedFor(String serial) {
    return 'Abfrage für $serial fehlgeschlagen.';
  }

  @override
  String get privacyPolicy => 'Datenschutzerklärung';

  @override
  String get publicKey => 'Öffentlicher Schlüssel';

  @override
  String get pushEndpointUrl => 'Push-Endpunkt URL';

  @override
  String get pushRequestParseError => 'Die Push-Anfrage konnte nicht verarbeitet werden.';

  @override
  String get pushToken => 'Push Token';

  @override
  String get qrFileDecodeError => 'Es war nicht möglich, den QR-Code aus dem ausgewählten Bild zu dekodieren. Bitte verwenden Sie stattdessen den QR-Code-Scanner.';

  @override
  String get qrInFileNotFound => 'Es wurde kein QR-Code in dem ausgewählten Bild gefunden.';

  @override
  String get qrInFileNotFound2 => 'Sie können mir zeigen, wo sich der QR-Code befindet.';

  @override
  String get qrInFileNotFound3 => 'Ich erwarte, dass ich den Code finde, wenn er sich in der Mitte des markierten Bereichs befindet.';

  @override
  String get qrNotFound => 'Kein QR-Code gefunden!';

  @override
  String get rename => 'Umbenennen';

  @override
  String get renameToken => 'Token umbenennen';

  @override
  String get renameTokenFolder => 'Ordner umbenennen';

  @override
  String get replaceButton => 'Ersetzen';

  @override
  String requestInfo(String account, String issuer) {
    return 'Gesendet von $issuer für Ihr Konto: \"$account\"';
  }

  @override
  String get requestPushChallengesPeriodically => 'Fordert regelmäßig Push-Anfragen vom Server an. Aktivieren Sie diese Funktion, wenn Nachrichten ansonsten nicht erhalten werden.';

  @override
  String get requestTriggerdByUserQuestion => 'Wurde diese Anfrage von Ihnen ausgelöst?';

  @override
  String get retryRolloutButton => 'Erneut ausrollen';

  @override
  String get rolloutStateCompleted => 'Ausrollen abgeschlossen';

  @override
  String get rolloutStateGeneratingKeyPair => 'Generiere Schlüsselpaar';

  @override
  String get rolloutStateGeneratingKeyPairCompleted => 'Generieren des Schlüsselpaars abgeschlossen';

  @override
  String get rolloutStateGeneratingKeyPairFailed => 'Generieren des Schlüsselpaars fehlgeschlagen';

  @override
  String get rolloutStateNotStarted => 'Starte Rollout';

  @override
  String get rolloutStateParsingResponse => 'Analysiere Antwort';

  @override
  String get rolloutStateParsingResponseCompleted => 'Analysieren der Antwort abgeschossen';

  @override
  String get rolloutStateParsingResponseFailed => 'Analysieren der Antwort fehlgeschlagen';

  @override
  String get rolloutStateSendingPublicKey => 'Sende öffentlichen Schlüssel';

  @override
  String get rolloutStateSendingPublicKeyCompleted => 'Senden des öffentlichen Schlüssels abgeschlossen';

  @override
  String get rolloutStateSendingPublicKeyFailed => 'Senden des öffentlichen Schlüssels fehlgeschlagen';

  @override
  String get saveButton => 'Speichern';

  @override
  String get scanQrCode => 'QR-Code scannen';

  @override
  String get scanThisQrWithNewDevice => 'Scannen Sie diesen QR-Code mit Ihrem neuen Gerät, um das Token zu importieren.';

  @override
  String get secondsUntilNextOTP => 'Sekunden bis zum nächsten OTP';

  @override
  String get secretIsRequired => 'Secret ist erforderlich';

  @override
  String get secretKey => 'Geheimer Schlüssel';

  @override
  String get selectFile => 'Datei auswählen';

  @override
  String get selectImportSource => 'Importquelle auswählen';

  @override
  String get selectImportType => 'Wie wollen Sie die Token importieren?';

  @override
  String selectTokensToExport(num count) {
    String _temp0 = intl.Intl.pluralLogic(
      count,
      locale: localeName,
      other: 'Wählen Sie die zu exportierenden Tokens aus',
      one: 'Wählen Sie das zu exportierende Token aus',
      zero: '',
    );
    return '$_temp0';
  }

  @override
  String get selectTokensToExportHelpContent1 => 'Wenn ein Token nicht aufgelistet ist, ist nicht garantiert, dass es sich nicht um ein privacyIDEA-Token handelt.';

  @override
  String get selectTokensToExportHelpContent2 => 'Zurzeit können nur manuell hinzugefügte und importierte Token exportiert werden.';

  @override
  String get selectTokensToExportHelpContent3 => 'Wir arbeiten an einer Lösung, um zwischen privacyIDEA-Tokens und privaten Tokens zu unterscheiden.';

  @override
  String get selectTokensToExportHelpContent4 => 'Einen neuen QR-Code erhalten Sie bei dem Dienst, von dem Sie den Token erhalten haben.';

  @override
  String get selectTokensToExportHelpTitle => 'Ist Ihr Token nicht aufgelistet?';

  @override
  String get send => 'Senden';

  @override
  String get sendErrorDialogBody => 'Ein unbekannter Fehler ist aufgetreten. Die unten gezeigten Informationen können den Entwicklern per E-Mail zugesendet werden, um zu helfen, diesen Fehler in Zukunft zu vermeiden.';

  @override
  String get sendErrorLogDescription => 'Es wird eine vorgefertigte E-Mail erstellt.\nSie enthält Informationen über die App, den Fehler und das Gerät.\nSie können die E-Mail vor dem Senden bearbeiten.\nWie wir die Informationen verwenden, sehen Sie hier:';

  @override
  String get sendPushRequestResponseFailed => 'Senden der Antwort fehlgeschlagen.';

  @override
  String get serverNotReachable => 'Der Server konnte nicht erreicht werden.';

  @override
  String get settings => 'Einstellungen';

  @override
  String get settingsGroupGeneral => 'Allgemeines';

  @override
  String get showDetails => 'Details anzeigen';

  @override
  String get showErrorLog => 'Anzeigen';

  @override
  String get showPrivacyPolicy => 'Datenschutzerklärung anzeigen';

  @override
  String get signInTitle => 'Authentifizierung wird benötigt';

  @override
  String get someTokensDoNotSupportPolling => 'Einige der Token sind veraltet und unterstützen keine aktiven Anfragen';

  @override
  String statusCode(int statusCode) {
    return 'Statuscode: $statusCode';
  }

  @override
  String get sync => 'Sync';

  @override
  String get syncContainerFailed => 'Container synchronisation fehlgeschlagen';

  @override
  String get syncFbTokenFailed => 'Synchronisation ist für die folgenden Token fehlgeschlagen:';

  @override
  String get syncFbTokenManuallyWhenNetworkIsAvailable => 'Bitte synchronisieren Sie die Push Token über die Einstellungen manuell, wenn eine Netzwerkverbindung verfügbar ist.';

  @override
  String get syncState => 'Sync-Status';

  @override
  String get syncStateCompletedDescription => 'Sync abgeschlossen';

  @override
  String get syncStateFailedDescription => 'Sync fehlgeschlagen';

  @override
  String get syncStateNotStartedDescription => 'Synchronisierung nicht gestartet';

  @override
  String get syncStateSyncingDescription => 'Aktuell wird synchronisiert';

  @override
  String get synchronizePushTokens => 'Synchronisiere Push Token';

  @override
  String get synchronizesTokensWithServer => 'Synchronisiert Token mit dem privacyIDEA Server.';

  @override
  String get synchronizingTokens => 'Synchronisiere Token.';

  @override
  String get theSecretDoesNotFitTheCurrentEncoding => 'Das Geheimnis entspricht nicht der gewählten Verschlüsselung.';

  @override
  String get theme => 'Farbschema';

  @override
  String get timeOut => 'Zeitüberschreitung';

  @override
  String get tokenDataParseError => 'Token-Daten konnten nicht geparst werden';

  @override
  String get tokenDetails => 'Tokendetails';

  @override
  String get tokenLinkImport => 'Token Link';

  @override
  String get tokenSerial => 'Token serial';

  @override
  String get tokensAreEncrypted => 'Die Token sind verschlüsselt. Bitte gib das Passwort ein, um sie zu entschlüsseln.';

  @override
  String get tokensDoNotSupportSynchronization => 'Die folgenden Token unterstützen keine Synchronisation und müssen erneut ausgerollt werden:';

  @override
  String get tokensSuccessfullyDecrypted => 'Die Token wurden erfolgreich entschlüsselt, sie können nun importiert werden.';

  @override
  String get type => 'Art';

  @override
  String get unexpectedError => 'Ein unerwarteter Fehler ist aufgetreten.';

  @override
  String get unknown => 'Unbekannt';

  @override
  String get unlock => 'Entsperren';

  @override
  String unsupported(String name, String value) {
    return 'Der $name [$value] wird von dieser Version der App nicht unterstützt.';
  }

  @override
  String get useDeviceLocaleDescription => 'Nutze Systemsprache, falls diese unterstützt wird. Anderenfalls nutze Englisch. ';

  @override
  String get useDeviceLocaleTitle => 'Nutze Systemsprache';

  @override
  String valueNotAllowed(String parameter, String type, String value) {
    return 'Typ $type „$value“ ist kein zulässiger Wert für „$parameter“';
  }

  @override
  String valueNotAllowedIn(String map, String parameter, String type, String value) {
    return 'Typ $type „$value“ ist kein zulässiger Wert für „$parameter“ in „$map“';
  }

  @override
  String get verboseLogging => 'Ausführliche Protokollierung';

  @override
  String get versionTitle => 'Version';

  @override
  String get wrongPassword => 'Falsches Passwort';

  @override
  String get yes => 'Ja';
}
