...
class DragItemScroller extends StatefulWidget {
  ...
  static const minScrollingSpeedDetectDistanceTop =
      40.0; // px distance to top it starts to scroll up
  ...
  static const minScrollingSpeedDetectDistanceBottom =
      120.0; // px distance to bottom it starts to scroll down
  ...
  const DragItemScroller({
    ...
    super.key,
         ...
         nestedScrollViewKey != null || scrollController != null,
         'Either nestedScrollViewKey or scrollController must be set',
       );
...
class _DragItemScrollerState extends State<DragItemScroller> {
  ...
  bool canScroll(ScrollController? controller) =>
      controller != null &&
      ((controller.offset > 0 && currentSpeed < 0) ||
  ...
  void _scrollJump() {
    ...
    final innerController =
        widget.nestedScrollViewKey?.currentState?.innerController ??
        widget.scrollController;
    final outerController =
        widget.nestedScrollViewKey?.currentState?.outerController;
    if (canScroll(outerController)) {
      final distanceOneFrame =
          currentSpeed / DragItemScroller.refreshRate; // px this frame
      final nextPosition = clampDouble(
        outerController!.offset + distanceOneFrame,
        0,
    ...
    if (canScroll(innerController)) {
      final distanceOneFrame =
          currentSpeed / DragItemScroller.refreshRate; // px this frame
      final nextPosition = clampDouble(
        innerController!.offset + distanceOneFrame,
        0,
  ...
  void _startScrolling(double speedInPercent, {bool moveUp = false}) {
    ...
    if (globalRef?.read(dragItemScrollerStateProvider.notifier).state != true &&
        currentSpeed != 0) {
      globalRef?.read(dragItemScrollerStateProvider.notifier).state =
          true; // set scrolling state to true if there is speed
    }
  }
  ...
  void _stopScrolling() {
    ...
    if (globalRef?.read(dragItemScrollerStateProvider.notifier).state == true) {
      globalRef?.read(dragItemScrollerStateProvider.notifier).state =
          false; // set scrolling state to false if there is no speed
    }
  }
    ...
    return Listener(
      ...
      onPointerMove: (PointerMoveEvent event) {
        ...
        final innerController =
            widget.nestedScrollViewKey?.currentState?.innerController ??
            widget.scrollController;
        final innerControllerOffset =
            innerController?.offset ?? widget.scrollController?.offset ?? 0.0;
        final innerControllerMaxScrollExtent =
            innerController?.position.maxScrollExtent ??
            widget.scrollController?.position.maxScrollExtent ??
            0.0;
        final outerControllerOffset =
            widget.nestedScrollViewKey?.currentState?.outerController.offset ??
            0.0;
        final outerControllerMaxScrollExtent =
            widget
                .nestedScrollViewKey
                ?.currentState
                ?.outerController
        ...
        final minScrollingSpeedDetectDistanceTopWithOuterOffset =
            DragItemScroller.minScrollingSpeedDetectDistanceTop +
            outerControllerOffset;
        if (event.position.dy <
                topY + minScrollingSpeedDetectDistanceTopWithOuterOffset &&
            (innerControllerOffset > 0 || outerControllerOffset > 0)) {
          ...
          final distanceToMaxSpeed =
              distanceToTop -
              (minScrollingSpeedDetectDistanceTopWithOuterOffset -
                  DragItemScroller.maxSpeedZoneHeightTop);
          final scrollSpeedPercent =
              1 - distanceToMaxSpeed / DragItemScroller.maxSpeedZoneHeightTop;
        ...
        if (event.position.dy >
                bottomY -
                    DragItemScroller.minScrollingSpeedDetectDistanceBottom &&
            (innerControllerOffset < innerControllerMaxScrollExtent ||
                outerControllerOffset < outerControllerMaxScrollExtent)) {
          // scroll down if the pointer is in the bottom range and the scrollController is not at the bottom
          final distanceToBottom =
              bottomY -
              event.position.dy; // distance to bottom of the widget in px
          final distanceToMaxSpeed =
              distanceToBottom -
              (DragItemScroller.minScrollingSpeedDetectDistanceBottom -
          ...
          final scrollSpeedPercent =
              1 -
              distanceToMaxSpeed / DragItemScroller.maxSpeedZoneHeightBottom;
          Logger.info('scrollSpeedPercent: $scrollSpeedPercent');
          _startScrolling(clampDouble(scrollSpeedPercent, 0.0, 1.0));
          return;
        }
        _stopScrolling();
      },
