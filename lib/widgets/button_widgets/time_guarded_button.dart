...
class _TimeGuardedButtonState extends State<TimeGuardedButton>
  ...
  void _handlePress() {
    if (widget.cooldownMs > 0) setState(() => _isCooldown = true);

    widget.onPressed?.call();

    if (widget.cooldownMs > 0 && mounted) {
      Future.delayed(Duration(milliseconds: widget.cooldownMs)).then((_) {
  ...
  FutureOr<void> Function()? get effectiveOnPressed {
    if (widget.onPressed == null || _isCooldown) return null;
