...
class SecureTokenRepository implements TokenRepository {
  ...
  Future<List<Token>> loadTokens() async {
    ...
    try {
      ...
      Logger.warning(
        'Could not migrate legacy tokens',
        error: e,
        verbose: true,
      );
    }
    try {
      keyValueMap = await _storage
          .readAll(); // Now only reads tokens with the correct prefix.
    } on PlatformException catch (e, s) {
      Logger.warning(
        "Token found, but could not be decrypted.",
        error: e,
        stackTrace: s,
        verbose: true,
      );
  ...
  Future<bool> saveOrReplaceToken(Token token) async {
    try {
      ...
      Logger.warning(
        'Could not save token to secure storage',
        error: e,
        verbose: true,
      );
      return false;
  ...
  Future<List<T>> deleteTokens<T extends Token>(List<T> tokens) async {
    ...
    if (failedTokens.isNotEmpty) {
      Logger.warning(
        'Could not delete all tokens from secure storage',
        error: 'Failed tokens: $failedTokens',
        stackTrace: StackTrace.current,
      );
    }
    return failedTokens;
  ...
  Future<bool?> _decryptErrorDeleteTokenConfirmationDialog() =>
      showAsyncDialog<bool>(
        builder: (context) => DefaultDialog(
          title: Text(AppLocalizations.of(context)!.decryptErrorTitle),
          content: Text(
            AppLocalizations.of(context)!.decryptErrorDeleteConfirmationContent,
          ),
          actions: [
            DialogAction(
