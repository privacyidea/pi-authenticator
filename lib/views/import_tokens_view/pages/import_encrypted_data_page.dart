...
class ImportEncryptedDataPage<T, V extends String?> extends StatefulWidget {
  ...
  State<ImportEncryptedDataPage> createState() =>
      _ImportEncryptedDataPageState();
}
...
class _ImportEncryptedDataPageState extends State<ImportEncryptedDataPage> {
    ...
    body: Center(
      child: SingleChildScrollView(
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              const Icon(Icons.file_present, size: ImportTokensView.iconSize),
              ...
              processingFuture != null
                  ...
                  : SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed:
                            _passwordController.text.isEmpty || wrongPassword
                            ? null
                            : () async {
                                setState(() {
                                  processingFuture =
                                      Future<void>(() async {
                                        try {
                                          final processorResults = await widget
                                              .processor
                                              .processTokenMigrate(
                                                widget.data,
                                          ...
                                          _pushImportPlainTokensPage(
                                            processorResults,
                                          );
                                        } on BadDecryptionPasswordException catch (
                                          _
                                        ) {
                                          setState(() {
                                            wrongPassword = true;
  ...
  Future<void> _pushImportPlainTokensPage(
