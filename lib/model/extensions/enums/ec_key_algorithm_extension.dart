...
extension EcKeyAlgorithmX on EcKeyAlgorithm {
  static final validator = RequiredObjectValidator<EcKeyAlgorithm>(
    transformer: (v) {
      if (v is EcKeyAlgorithm) return v;
      if (v is String) {
        return EcKeyAlgorithm.values.firstWhere(
          (alg) => alg.curveName == v,
          orElse: () => throw ArgumentError('Unknown curve name: $v'),
        );
      }
      throw ArgumentError('Invalid type for EcKeyAlgorithm: ${v.runtimeType}');
    },
  );
